...
from typing import Dict, List, Union, Collection, Any
...
class Scope(ABC):
    ...
    class Type:
        def __init__(self, type: 'Scope.InnerType', wrappedType: 'Scope.Type' = None):
            self.type = type
            self.wrappedType = wrappedType

        @staticmethod
        def pointerToType(wrappedType: 'Scope.Type') -> 'Scope.Type':
            return Scope.Type(Scope.InnerType.PTR, wrappedType)

        def getWrappedType(self) -> 'Scope.Type':
            return self.wrappedType

        def __str__(self):
            if self.type is not Scope.InnerType.PTR:
                return str(self.type)
            else:
                return "PTR to " + str(self.wrappedType)
        
        def __eq__(self, other: Any) -> bool:
            if not isinstance(other, Scope.Type):
                return False
            if self.type is not Scope.InnerType.PTR:
                return self.type == other.type
            else:
                return self.wrappedType == other.getWrappedType()

        def __ne__(self, other: 'Scope.Type') -> bool:
