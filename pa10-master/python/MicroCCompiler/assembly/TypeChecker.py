...
class CodeGenerator(AbstractASTVisitor):
  ...
  def postprocessAssignNode(self, node: AssignNode, left: CodeObject, right: CodeObject) -> CodeObject:
    #print("Processing Assign Node\n")
    #print("Left: ", left, left.type, left.temp)
    #print("Right: ", right, right.type, right.temp)
    if (left.type != right.type):
      print("TYPE ERROR", file=sys.stderr)
      sys.exit(7)
    
    #address = symbol.addressToString()
    #print("Processing Assign with left: ", left)
    #print("Left is var? ", left.isVar())
    #print("Left is local?", symbol.isLocal())
    #Step 0: create new code object
    
    # Is RHS a function node?
    #print("RHS is of type: " + str(right.type))
    #print("RHS info: " + str(right.funcName))


    co = CodeObject()
    #co.code.append("; RHS is type: " + str(right.type))
    #print("RHS is type: " + str(right.type))
    ...
    ### ADD NEW CODE TO HANDLE LOCAL VARIABLES SEPARATE FROM GLOBAL

    #Step 1a
    if left.isVar() and (not symbol.isLocal()): # Handles global variables
      #print("Left is var: ")
      #print("Left before addressing: ", left)
      newaddrtemp = self.intTempPrefix + str(self.intRegCount)
      newleft = self.generateAddrFromVariable(left)
      #print("new left is: ", newleft)
      instrlist.extend(newleft)
      instrlist.extend(right.code)
      if left.type == Scope.Type.INT:
        newcode = Sw(right.temp, newaddrtemp, "0")
      elif left.type == Scope.Type.FLOAT:
        #print("Processing store of float!\n")
        newcode = Fsw(right.temp, newaddrtemp, "0")
        #print("newcode: ", newcode)
    if left.isVar() and symbol.isLocal():
      #newleft = self.generateAddrFromVariable(left) # gets address
      address = symbol.addressToString() # Gets address as offset from FP
      instrlist.extend(right.code)
      if left.type == Scope.Type.INT:
        newcode = Sw(right.temp, "fp", address)
      elif left.type == Scope.Type.FLOAT:
        newcode = Fsw(right.temp, "fp", address)
  ...
  def postprocessCondNode(self, node: CondNode, left: CodeObject, right: CodeObject) -> CodeObject:
    ...
    #print(";Processing Conditional Node\n")
    #print(";Left temp:", left.temp, "Left type:", left.type, "Left lval:", left.lval)
    if(left.lval == True):
      left = self.rvalify(left)
    if(right.lval == True):
      right = self.rvalify(right)
    # General idea for this: we have the code for left and right objects.  First make sure their types
    # match (int or float).  For int, mark the code object as type int for use in if/while node code; otherwise
    # mark as float.  
    # 
    # If int, stitch the two code objects together, but then we need the left/right temps to pass forward.
    # For now, I've added temp2 as a property of code objects, so this node can have a temp (left side) and temp2 (right side)
    #   
    # If float, we have to do two steps: a comparison or series of comparisons with feq.s, flt.s, and fle.s
    # These assign a 1 or 0 to a target register based on comparing two temp registers (f registers)
    # Does the result need to go to an f register or t register?  Check the manual, but we'll handle ints only for now.
    # What needs to be rvalified?  Anything?

    co = CodeObject()
    #if left.type != right.type:
    #  print(";Error: left side of comparison type does not match right side!\n")
    #  return co
    if left.type != right.type:
      print("TYPE ERROR", file=sys.stderr)
      sys.exit(7)
  ...
  def postprocessIfStatementNode(self, node: IfStatementNode, cond: CodeObject, tlist: CodeObject, elist: CodeObject) -> CodeObject:
    ...
    revcmptype = self.reverseCompType(cond.type, cond.cmptype)
    ...
    if cond.type == Scope.Type.FLOAT: # condition is float-based!
      ...
      tempintreg = self.generateTemp(Scope.Type.INT) # Need integer temporary
  ...
  CodeObject) -> CodeObject:
    ...
    if cond.type == Scope.Type.INT: # condition is integer-based!  
  ...
  def postprocessReturnNode(self, node: ReturnNode, retExpr: CodeObject) -> CodeObject:
    ...
    #co.code.append("; Processing return node!")
    #co.code.append("; Return type is: " + str(node.type))
    #co.code.append("; Return expression is: " + str(retExpr.type))
    #Where am I?
    ...
    if retExpr.lval:
      ...
      #print("Rvalifying return expression!")
    # FILL IN
    #print(retExpr)
    ...
    # Generate jump?
    #co.code.append("; Generating function out label")
    co.code.append(J(self.generateFunctionOutLabel()))
    #print("; Return expr is type: " + str(retExpr.type))
  ...
  def postprocessCallNode(self, node: CallNode, args: List[CodeObject]) -> CodeObject:
    ...
    co.code.append("; Call node has type: " + str(node.type))
    ...
    newtemp = self.generateTemp(node.type)
  ...
  def rvalify(self, lco : CodeObject) -> CodeObject:
    # Need to add ability to handle local vars
    # Step 0
    co = CodeObject()
    symbol = lco.getSTE()
    symboladdress = symbol.addressToString()
    assert(lco.lval is True)
    #co.code.append("; rvalifying: " + str(symbol))
    #co.code.append("; is local? " + str(symbol.isLocal()))
    # FILL IN CODE FOR STEP 2
    #Step 1:
    #print("Rvalify input type: ", lco.type)
    if lco.isVar() and (not symbol.isLocal()):
      newaddrtemp = self.intTempPrefix + str(self.intRegCount)
    ...
    if lco.isVar() and symbol.isLocal():
  ...
  def reverseCompType(self, type: Scope.Type, cmptype: int) -> int:
    ...
    #print("Reversing: ", cmptype)
    if type == Scope.Type.INT:
      return intdictionary[cmptype]
    elif type == Scope.Type.FLOAT:
      return intdictionary[cmptype]
    else:
      print("Bad data type in reverseComp!\n")